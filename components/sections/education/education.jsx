		...
		<Section data={{ className: 'education', anchorId: data.anchorId }}>
			...
			<div className="education__item-wrapper">
				{ courses.map((item, index) => (
					<EducationItem key={ index } data={{ 	date: item.date, 	activities: item.activities, 	image: item.image }}/>
				))}
			</div>
		</Section>
	);
	...
	return (
		<div className="item">
			...
			<ul className="item__list">
				{data.activities && data.activities.map((item, index) => <li className="item__list-item" key={ index }>{ item }</li>) }
				{ data.image && <Image data={{ className: 'item__image', src: data.image.src, alt: data.image.alt, width: 700, height: 500 }}/> }
			</ul>
		</div>
	);
